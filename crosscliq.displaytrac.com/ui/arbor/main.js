    ...
    var that = {
      ...
      redraw:function(){
        ...
        sys.eachNode(function(node, pt){
          var w = Math.max(20, 10+gfx.textWidth(node.data.label) )
...
var server = sys.addNode('Server',{'id':'Server','color':'green','shape':'dot','label':'Sys','alpha':'1','link':''});
  ...
  if (data.type=='node') {
    ...
    var rootNode = sys.addNode(data.child,{'id':data.child,'color':data.color,'shape':'dot','label':data.name,'alpha':'.5','link':data.link});
    console.log(rootNode);
  } else { 
    var childNode = sys.addNode(data.child,{'id':data.child,'color':data.color,'label':data.name,'alpha':'.5','link':data.link});
